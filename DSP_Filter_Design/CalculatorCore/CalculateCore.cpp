...
QMap<TFZType, TFZ_coefficients> CalculateCore::MakeCoeffBandList(BandType type, double center_freq, double QValue, double Gain)
{
	QList<TFZType> list_type = { TFZType::k44_1,TFZType::k48,TFZType::k96,TFZType::k192 };
	...
	for (auto item : list_type) {
		...
		switch (type) {
		case BandType::P: {
			return this->MakeCoeffPeakFilterList(center_freq, QValue, Gain);
		}case BandType::LS: {
			return this->MakeCoeffLowShelfList(center_freq, QValue, Gain);
		}case BandType::HS: {
			return this->MakeCoeffHighShelfList(center_freq, QValue, Gain);
		}
		}
	}
	return map_rets;
...
QMap<TFZType, TFZ_coefficients> CalculateCore::MakeCoeffLowPassList(double center_freq)
{
	QMap<TFZType, TFZ_coefficients> map_ret;
	...
	for (auto item : list_type) {
		TFZ_coefficients ret;
		double C = 1 / std::tan(M_PI * center_freq / (double)item);
		double SqC = std::pow(C, 2);
		double MultC = sqrt(2) * C;
		double c = 1.0 / (1.0 + MultC + SqC);
...
QMap<TFZType, TFZ_coefficients> CalculateCore::MakeCoeffHighPassList(double center_freq)
{
	QMap<TFZType, TFZ_coefficients> map_ret;
