...
## C.7 小白版：把 GTConvBlock 当成“滤波器 + 两个旋钮”

如果你刚接触这个结构，可以先不看所有细节，先抓住这个直觉：

- **DWConv + PWConv**：像一个可学习的“时频滤波器”，负责提取模式
- **TRALite**：第一个旋钮，决定“哪些时间帧更该被强调”
- **SEBlock**：第二个旋钮，决定“哪些通道特征更有用”
- **残差连接**：保底通路，避免模块把原始有效信息改坏

所以一个 GTConvBlock 并不是“从零重建特征”，而是：

> 先提取一点新模式，再动态调节强弱，最后和原特征融合。
...
1) DWConv(k=3, d=4) + PWConv
   -> y1: [B, 32, T, 55]   （尺寸不变，特征被更新）

2) TRALite（按时间帧门控）
   -> y2: [B, 32, T, 55]   （每个时间帧乘上不同权重）

3) SEBlock（按通道门控）
   -> y3: [B, 32, T, 55]   （每个通道乘上不同权重）
...
你可以把它理解为“**同尺寸特征微调器**”：输入输出尺寸一致，重点是质量提升，不是分辨率变化。

---

## C.8 训练/部署时最常见的 5 个坑

### 1) dilation 变了但 padding 没改

如果 `k=3, d=d`，想保持尺寸不变（`s=1`），通常要满足对应轴上的 `p=d`。否则会出现时频尺寸漂移，skip connection 对不上。

### 2) 因果模式下还在用对称 padding

V3 实时模型要求时间轴只看过去。若时间轴仍左右对称 pad，会泄漏未来信息，离线指标好看但流式失真明显。

### 3) 门控顺序乱改

常见顺序是：卷积提特征 → 时间门控（TRALite）→ 通道门控（SE）。
随意交换顺序不一定报错，但会影响稳定性与可解释性。

### 4) 忘了残差

去掉残差后，深层堆叠更容易训练不稳，且收敛慢。GTCRN 的这类轻量块通常依赖残差来保证梯度流动。

### 5) 只看参数量，不看感受野覆盖

GTConvBlock 轻量化的同时，必须保证膨胀率序列能覆盖到足够长的上下文。
只减参数不看感受野，会导致“模型更小了，但听感变差很多”。
...
> 用轻量卷积提取局部时频模式，再用时间门控 + 通道门控做动态重标定，最后通过残差稳定地叠加到主干特征上。

它负责“局部而高效”的建模；全局依赖则交给后面的 DPGRNN。
